...
file_path = r'c:\Users\user\Desktop\DIPLOM\booking_app\lib\main.dart'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Replace Pride with Continental
content = content.replace('PRIDE cyber club', 'Continental cyber club')
content = content.replace('🎮 Новая бронь в PRIDE!', '🎮 Новая бронь в Continental!')

# Now replace the _MapScreenState and MapPainter completely
new_map_screen_state = """class _MapScreenState extends State<MapScreen> {
  int? _selectedSeatId;
...
pattern = re.compile(r"class _MapScreenState extends State<MapScreen> \{.*?bool shouldRepaint\(covariant CustomPainter oldDelegate\) => false;\n\}", re.DOTALL)
content = pattern.sub(new_map_screen_state, content)
